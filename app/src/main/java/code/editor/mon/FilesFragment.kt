...
class FilesFragment : Fragment() {
    ...
    override fun onCreateView(i: LayoutInflater, c: ViewGroup?, s: Bundle?) =
        FragmentFilesBinding.inflate(i, c, false).also { _b = it }.root
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        b.btnOpenZip.setOnClickListener {
            zipPickLauncher.launch(Intent(Intent.ACTION_OPEN_DOCUMENT).apply {
                addCategory(Intent.CATEGORY_OPENABLE)
                type = "application/zip"
            })
        }
        b.btnExportZip.setOnClickListener {
            if (!ProjectManager.isProjectOpen()) { toast("No project open"); return@setOnClickListener }
            exportZipLauncher.launch(Intent(Intent.ACTION_CREATE_DOCUMENT).apply {
                addCategory(Intent.CATEGORY_OPENABLE)
                type = "application/zip"
    ...
    private fun loadSafProject(uri: Uri) {
        ...
        lifecycleScope.launch {
            val root = withContext(Dispatchers.IO) { DocumentFile.fromTreeUri(requireContext(), uri) }
            if (root == null || !root.isDirectory) { setLoading(false); toast("Cannot open folder"); return@launch }
            val nodes = withContext(Dispatchers.IO) { FileUtils.traverseDocumentFile(root) }
    ...
    private fun loadZip(uri: Uri) {
        ...
        lifecycleScope.launch {
            val dir = withContext(Dispatchers.IO) {
                val dest = File(requireContext().getExternalFilesDir("projects"),
                    "proj_${System.currentTimeMillis()}").also { it.mkdirs() }
                if (FileUtils.extractZip(uri, dest, requireContext())) dest else null
            }
            if (dir == null) { setLoading(false); toast("Failed to extract ZIP"); return@launch }
            val root = dir.listFiles()?.takeIf { it.size == 1 && it[0].isDirectory }?.get(0) ?: dir
        ...
        lifecycleScope.launch {
            ...
            val ok = withContext(Dispatchers.IO) {
                if (ProjectManager.isLocalProject)
                    ProjectManager.localProjectDir?.let { FileUtils.zipLocalDirectory(it, destUri, requireContext()) } ?: false
                else
                    ProjectManager.rootDocument?.let { FileUtils.zipDocumentFolder(it, destUri, requireContext()) } ?: false
            }
            setLoading(false)
    ...
    private fun openFile(node: FileNode) {
        if (!FileUtils.isTextFile(node.name)) { toast("Binary file — cannot edit"); return }
        vm.openFileRequest.value = FileOpenRequest(
            uri = node.uri, fileName = node.name,
            relativePath = node.relativePath, isLocal = ProjectManager.isLocalProject
        )
        findNavController().navigate(R.id.toEditor)
    }

    private fun showSettings() {
        val items = arrayOf("Close Project")
        android.app.AlertDialog.Builder(requireContext())
            .setTitle("Settings")
            .setItems(items) { _, which ->
                when (which) {
                    0 -> { ProjectManager.reset(); showEmpty() }
                }
            }.show()
    }
    ...
    private fun setLoading(on: Boolean) {
        ...
        if (on) { b.layoutProjectBar.visibility = View.VISIBLE; b.tvProjectName.text = "Loading..." }
    }
